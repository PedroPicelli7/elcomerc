...
export default function UserOrdersPage() {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case "pending": return "text-brand-cyan border-brand-cyan/20 bg-brand-cyan/5";
  ...
  return (
    <>
      <Header />
      <main className="min-h-screen bg-neutral-950 px-4 py-8 sm:px-6 lg:px-8 mx-auto w-full max-w-4xl space-y-8 animate-in fade-in duration-500">
        
        <div>
          <Link href="/" className="flex items-center gap-1 font-mono text-xs text-neutral-500 hover:text-brand-cyan transition-colors">
        ...
        <div className="border-b border-neutral-800 pb-5">
          <h1 className="text-2xl font-black text-white font-mono uppercase flex items-center gap-2">
            <ShoppingBag className="text-brand-cyan h-6 w-6" /> Minhas <span className="text-brand-cyan">Compras</span>
          </h1>
        ...
        {loadingOrders ? (
          ...
          <div className="text-center py-16 border border-dashed border-neutral-800 rounded-2xl p-6">
            ...
            <Link 
              href="/" 
              className="mt-6 inline-block rounded border border-brand-cyan/20 bg-brand-cyan/10 px-4 py-2 font-mono text-xs font-bold text-brand-cyan hover:bg-brand-cyan/20 transition-all cursor-pointer"
            >
              EXPLORAR CATÁLOGO
            </Link>
          </div>
              ...
              >
                ...
                <div className="border-t border-neutral-800/40 pt-3 flex justify-between items-center font-mono text-xs">
                  ...
                  <div className="text-neutral-400 text-[11px]">
                    Valor Pago: <strong className="text-sm font-black text-brand-cyan ml-1">{order.total.toLocaleString("pt-BR", { style: "currency", currency: "BRL" })}</strong>
                  </div>
                </div>
