...
export default function RegisterPage() {
  ...
  const handleGoogleRegister = async () => {
    try {
      ...
      const { error } = await supabaseClient.auth.signInWithOAuth({
        provider: "google",
        options: {
          // Após o login no Google, ele joga o cliente de volta para a Home da Elcomerc
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-neutral-950 px-4 py-12 sm:px-6 lg:px-8">
      ...
      <div className="w-full max-w-md space-y-8 rounded-2xl border border-neutral-800 bg-neutral-900 p-8 shadow-2xl">
        ...
        <div className="flex flex-col items-center text-center">
          ...
          <h2 className="mt-6 text-2xl font-black tracking-tight text-white font-mono uppercase">
            Criar sua conta na <span className="text-orange-500">Elcomerc</span>
          </h2>
          <p className="mt-2 text-sm text-neutral-400">
            Acesse o catálogo completo e gerencie seus pedidos instantaneamente.
          </p>
        </div>
        ...
        {/* Ação Principal: Botão do Google OAuth */}
        <div className="mt-8 space-y-4">
          ...
          >
            {isLoading ? (
              ...
              /* Ícone do Google em SVG Puro e Colorido para destacar no Dark Mode */
            ...
            {isLoading ? "Conectando..." : "Cadastrar com o Google"}
          </button>
        </div>
      ...
      {/* Link rápido de escape para voltar à loja sem logar */}
      <Link
        href="/"
