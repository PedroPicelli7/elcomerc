...
// 1. IMPORTA O TIPO OFICIAL QUE O SEU CONTEXTO EXIGE
...
export function ProductCatalog() {
  ...
  // Carrega os produtos em tempo real do banco de dados
  useEffect(() => {
    async function fetchLiveProducts() {
      try {
        setLoading(true);
        // Trazemos todas as colunas necessárias, incluindo a category_id para satisfazer o tipo global
    ...
    <section className="w-full">
      ...
      <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-2 text-xs font-mono text-neutral-400">
          ...
          <span>
            {loading ? "Sincronizando..." : `${filteredProducts.length} ${filteredProducts.length === 1 ? "produto encontrado" : "produtos encontrados"}`}
          </span>
        </div>
      </div>
      ...
      {loading ? (
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="h-80 w-full animate-pulse rounded-lg border border-neutral-800 bg-neutral-900/40" />
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {filteredProducts.map((product) => {
            ...
            return (
              ...
              >
                <Link href={`/produto/${product.slug}`} className="aspect-square w-full overflow-hidden rounded-md bg-neutral-950">
                  <img
                ...
                <div className="flex flex-1 flex-col pt-4">
                  ...
                  <Link href={`/produto/${product.slug}`} className="mt-1 block">
                    <h3 className="text-sm font-bold text-neutral-100 group-hover:text-brand-cyan transition-colors line-clamp-1 font-mono">
                      {product.name}
                    </h3>
                  </Link>

                  <p className="mt-1 text-xs text-neutral-400 line-clamp-2 flex-1 font-mono">
                    {product.description}
                  </p>
        ...
        <div className="rounded-xl border border-dashed border-neutral-800 p-12 text-center w-full col-span-full">
          <p className="text-sm text-neutral-400 font-mono">Nenhum produto cadastrado nesta categoria.</p>
        </div>
      )}
    </section>
  );
