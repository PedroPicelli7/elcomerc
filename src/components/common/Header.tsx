  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b border-neutral-800 bg-neutral-950/80 backdrop-blur-md">
      <div className="container mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6">
        ...
        {/* Barra de Busca (Visível em Desktop) */}
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          ...
          {!loading && (
            <>
              {user ? (
                <div className="flex items-center gap-2 border-r border-neutral-800 pr-2 sm:pr-4">
                  <span className="hidden max-w-[120px] truncate font-mono text-xs text-neutral-300 md:block">
                    {user.user_metadata?.name?.split(" ")[0] || "Cliente"}
                  </span>
                  <button
                    onClick={logout}
                    className="rounded-md p-2 text-neutral-500 hover:text-red-400 transition-colors"
                    title="Sair da conta"
                  >
